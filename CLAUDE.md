...
- **Next (Phase C):** Kotlin `getUsage(packages, days)` via `UsageStatsManager` → replace mock usage on dashboard/stats; compute `baselineMinutes` from past-7-day query on first launch.
